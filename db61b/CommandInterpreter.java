...
class CommandInterpreter {
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            if (_input.nextIs(")")) {
                throw error("column should at least be 1");
            }
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            List<Condition> conditions = conditionClause(table1, table2);
            return table1.select(table2, columnNames, conditions);
        } else {
            List<Condition> conditions = conditionClause(table1);
    ...
    Condition condition(Table... tables) {
        Column col1 = new Column(_input.next(), tables);
        String relation = _input.next();
        if (_input.peek().substring(0, 1).equals("'")) {
            String val = literal();
            return new Condition(col1, relation, val);
        } else {
            Column col2 = new Column(_input.next(), tables);
